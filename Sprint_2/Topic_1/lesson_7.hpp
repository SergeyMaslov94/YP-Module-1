...
void TestDocumentMatching() 
{
    ...
    server.AddDocument(doc_id_3, document_3, DocumentStatus::ACTUAL, ratings);
    {
        ...
        assert(words[0] == "kot"s);
        assert(words[1] == "modnyj"s);
    }
}
...
void TestSortByRelevance() 
{
    ...
    assert(results[1].id == doc_id_4);
    assert(results[2].id == doc_id_3);
}
...
void TestSearchDocumentsByStatus()
{
    ...
    // вернет только ACTUAL результат
    {
        ...
        assert(results.size() == 2U);
        assert(results[0].id == doc_id_1);
        assert(results[1].id == doc_id_3);
    }
